...
const sendTokenResponse = async (user, statusCode, req, res) => {
  ...
  // Use req.secure (respects trust-proxy / X-Forwarded-Proto) instead of
  // NODE_ENV so the behaviour is driven by the actual transport, not an env
  // var that may not be set on the hosting platform.
...
export const logout = async (req, res) => {
  ...
  res
    .status(200)
    .clearCookie('token', { httpOnly: true, secure, sameSite })
    .clearCookie('refreshToken', { httpOnly: true, secure, sameSite })
