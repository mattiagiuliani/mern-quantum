...
describe('register', () => {
  ...
  it('creates user, sets cookie and returns safe user on success', async () => {
    ...
    try {
      const req = { body: { username: 'testuser', email: 'test@example.com', password: 'secret123' }, headers: {}, secure: false }
      const res = createRes()
      await register(req, res)
...
describe('login', () => {
  ...
  it('sets cookie and returns safe user on success', async () => {
    ...
    User.findByIdAndUpdate = async () => {}
    try {
      const req = { body: { email: 'test@example.com', password: 'correct-password' }, headers: {}, secure: false }
      const res = createRes()
      await login(req, res)
