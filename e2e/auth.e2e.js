...
test('register flow redirects to builder on success', async ({ page }) => {
  ...
  // Use .first() because 'My Circuits' appears in both the desktop nav and the
  // hidden mobile nav — strict mode would fail if we don't disambiguate.
  await expect(page.getByRole('button', { name: 'My Circuits' }).first()).toBeVisible({ timeout: AUTH_REDIRECT_TIMEOUT_MS })
})
