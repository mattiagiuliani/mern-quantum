...
import { calculatePercentage } from '../../multi-run/multiRun.utils'
        ...
        <div key={rowIdx} style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {row.map((gate, stepIdx) => {
              const label = gate == null ? null
                : typeof gate === 'object' ? (gate.role === 'ctrl' ? '●' : '⊕')
                : gate
              return (
              <div
                key={`${rowIdx}-${stepIdx}`}
                style={{
                  ...
                  background: label ? 'rgba(110,231,208,0.12)' : 'rgba(255,255,255,0.03)',
                  color: label ? '#6EE7D0' : 'rgba(255,255,255,0.24)',
                  display: 'flex',
                  alignItems: 'center',
          ...
          <div style={{
            ...
            {previewStatus === 'error' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button
                  onClick={() => onRunPreview(template.circuit)}
                  style={{ fontSize: 11, color: '#6EE7D0', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}
                >
                  {TEMPLATE_COPY.previewRetry}
                </button>
              </div>
            )}
