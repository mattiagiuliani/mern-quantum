...
function QasmPreviewBlock({ circuitMatrix }) {
  ...
  for (let s = 0; s < stepCount; s++) {
    for (let q = 0; q < circuitMatrix.length; q++) {
      ...
      if (!g) continue
      // CNOT is stored as an object { gate:'CNOT', role:'ctrl'|'tgt', partner:number }
      if (typeof g === 'object' && g.gate === 'CNOT') {
        if (g.role === 'ctrl') lines.push(`cx q[${q}], q[${g.partner}];`)
        continue
      }
      if (g === 'H') { lines.push(`h q[${q}];`); continue }
      if (g === 'X') { lines.push(`x q[${q}];`); continue }
      if (g === 'S') { lines.push(`s q[${q}];`); continue }
      if (g === 'M') { lines.push(`measure q[${q}] -> c[${q}];`); continue }
    }
  }
...
export default function ResultsPage() {
  ...
  return (
    <div style={{
      ...
      color: '#F1EDE4',
      fontFamily: "'Space Mono', monospace",
      padding: 'clamp(72px, 10vh, 80px) clamp(16px, 4vw, 24px) 32px',
    }}>
